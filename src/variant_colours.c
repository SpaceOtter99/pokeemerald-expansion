...
void ApplyPaletteVariantToPaletteBuffer(u16 pal16[16], const struct PaletteVariant *pv, u16 prn16)
{
    ...
    for (u8 i = iStart; i <= iEnd; ++i) {
        ...
        if (pv->sv_down_only || downC) c = cDelta > c ? 0 : (u8)(c - cDelta);
        else c = (u8)(c + cDelta);

        if (pv->sv_down_only || downL) l = lDelta > l ? 0 : (u8)(l - lDelta);
        else l = (u8)((255 - lDelta < l) ? 255 : (l + lDelta));
